...
class SAC_Trainer():
    def __init__(self, replay_buffer, 
        ...
        self.q_net1 = QNetwork(state_space, action_space, critic_hidden_dim).to(self.device)
        self.q_net2 = QNetwork(state_space, action_space, critic_hidden_dim).to(self.device)
        self.target_q_net1 = QNetwork(state_space, action_space, critic_hidden_dim).to(self.device)
        self.target_q_net2 = QNetwork(state_space, action_space, critic_hidden_dim).to(self.device)
        self.policy_actf = activation
        self.policy_net = PolicyNetwork(state_space, action_space, actor_hidden_dim, device, self.policy_actf, out_actf, action_scale).to(self.device)
        self.target_policy_net = PolicyNetwork(state_space, action_space, actor_hidden_dim, device, self.policy_actf, out_actf, action_scale).to(self.device)
    ...
    def _setup_entropy(self):
        ...
        self.ent_coef_optimizer = torch.optim.Adam([self.log_ent_coef], lr=self.actor_lr)
    ...
    def get_action(self, state, last_action, *args, **kwargs):
        ...
        return self.policy_net.get_action(state, deterministic=deterministic), None
...
class SACparam_Trainer(SAC_Trainer):
    def __init__(self, replay_buffer, 
        ...
        self.param_optimizer = optim.Adam(self.param_net.parameters(), lr=self.lr)
