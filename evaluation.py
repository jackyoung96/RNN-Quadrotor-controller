    ...
    with Display(visible=False, size=(100, 60)) as disp:
        ...
        if 'Pendulum' in env_name:
            ...
            envs = domainRandomAviary(envs, 'test', 0, seed,
                ...
                episode_len_sec=2,
                max_rpm=66535,
                initial_xyz=[[0.0,0.0,2.0]], # Far from the ground
                freq=200,
                rpy_noise=1.2,
                vel_noise=1.0,
                angvel_noise=2.4,
        ...
        with torch.no_grad():
            for i_eval in range(test_itr):
                if 'Pendulum' in env_name:
                    ...
                    eval_env.seed(seed)
                    setattr(eval_env,'env_name', env_name)
                    domainRandomize(eval_env, dyn_range=dyn_range, seed=seed)
                elif 'aviary' in env_name:
                    max_steps = 4000
                    eval_env = gym.make(id=env_name, # arbitrary environment that has state normalization and clipping
                        drone_model=DroneModel.CF2X,
                        ...
                        obs=ObservationType.KIN,
                        act=ActionType.RPM)
                    eval_env = domainRandomAviary(eval_env, 'test', 0, seed,
                        observable=['pos', 'rotation', 'vel', 'angular_vel', 'rpm'],
                        frame_stack=1,
                        task='stabilize2',
                        reward_coeff={'xyz':0.2, 'vel':0.016, 'ang_vel':0.08, 'd_action':0.002},
                        episode_len_sec=2,
                        max_rpm=66535,
                        initial_xyz=[[0.0,0.0,1000.0]], # Far from the ground
                        freq=200,
                        rpy_noise=1.2,
                        vel_noise=1.0,
                        angvel_noise=2.4,
