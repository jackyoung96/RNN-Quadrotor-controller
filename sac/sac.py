...
class SACparam_Trainer(SAC_Trainer):
    ...
    def update(self, batch_size, gamma=0.99, soft_tau=1e-3):
        state, action, last_action, reward, next_state, done, *_ = self.replay_buffer.sample(batch_size)
...
class TD3RNN_Trainer2(SACRNN_Trainer):
    def __init__(self, replay_buffer, state_space, action_space, hidden_dim, param_dim, rnn_type='RNNpolicy', out_actf=None, action_scale=1.0, device='cpu', policy_target_update_interval=2, **kwargs):
        super().__init__(replay_buffer, state_space, action_space, hidden_dim, param_dim, rnn_type=rnn_type.strip('policy'), out_actf=out_actf, action_scale=action_scale,device=device, policy_target_update_interval=policy_target_update_interval, **kwargs)
    ...
    def update(self, batch_size, norm_ftn, deterministic, eval_noise_scale, gamma=0.99, soft_tau=1e-3):
        ...
        with torch.no_grad():
            ...
            target_q_1, _ = self.target_q_net1(next_state, new_next_action)
            target_q_2, _ = self.target_q_net2(next_state, new_next_action)
            target_q_min = torch.min(target_q_1, target_q_2)
            target_q_min = target_q_min - ent_coef * next_log_prob.reshape(-1,1)
            target_q_value = reward + (1 - done) * gamma * target_q_min # if done==1, only reward

        predicted_q_value1, _ = self.q_net1(state, action[:,-1])
        predicted_q_value2, _ = self.q_net2(state, action[:,-1])
        ...
        # Actor update
        predicted_new_q_value, _ = self.q_net1(state[:,-1], new_action)
        policy_loss = (ent_coef * log_prob - predicted_new_q_value).mean()
        self.policy_optimizer.zero_grad()
        policy_loss.backward()
        self.policy_optimizer.step()
...
class TD3RNN_Trainer3(SACRNN_Trainer):
...
class TD3HERRNN_Trainer(SACRNN_Trainer):
...
class TD3sHERRNN_Trainer(SACRNN_Trainer):
...
class TD3FastAdaptRNN_Trainer(SACRNN_Trainer):
